...
export const AdminFlightManager: React.FC<AdminFlightManagerProps> = ({ flights, onAddFlight, onDeleteFlight }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Backend ต้องการ CreateFlightDto (camelCase บาง field)
      const payload = {
        ...
        availableSeats: Number(newFlight.available_seats) || 0,
      }
    ...
    alert('✅ เพิ่มเที่ยวบินเรียบร้อย!');
      ...
      alert('❌ เพิ่มเที่ยวบินไม่สำเร็จ (กรุณาตรวจสอบว่า Login เป็น ADMIN และ backend ทำงานอยู่)')
    }
  };
